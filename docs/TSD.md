...
6. 백엔드 → Bearer 토큰 발급 → 프론트 localStorage 저장
```
...
│   └── security.py              # 토큰 암호화 유틸 (itsdangerous)
...
│   ├── task.py
│   ├── subtask.py
│   ├── task_tag.py              # 태스크 태그 (다대다 대신 단순 목록)
...
| PATCH | `/tasks/{id}` | 태스크 수정 (담당자·태그·우선순위·마감일 등) |
...
2. 백엔드:
   ...
   c. 로컬 DB에 저장 (gcs_reservation_id, google_calendar_event_id)
...
- 백엔드: Railway — `uvicorn app.main:app --host 0.0.0.0 --port $PORT`
- 프론트엔드: Railway Static 또는 Vercel — `pnpm build` 후 `dist/` 서빙
- SQLite 파일은 Railway Volume 마운트 (영속성 보장)
- HTTPS는 Railway/Vercel이 자동 처리
