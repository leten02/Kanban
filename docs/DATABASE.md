...
- **DBMS**: PostgreSQL (Railway managed)
- **ORM**: SQLAlchemy 2.x (async, asyncpg)
- **연결**: `DATABASE_URL` 환경변수로 관리 (Railway 자동 주입)
- **마이그레이션**: `create_all()` (앱 시작 시 자동 테이블 생성, Alembic 미사용)
...
- Alembic 미사용
- `backend/app/models/` 모델 변경 후 앱 재시작 시 `create_all()`이 자동으로 테이블 생성
- 컬럼 추가/변경은 Railway에서 직접 SQL 실행하거나 `alter table` 명령 필요
- 로컬 개발: `DATABASE_URL=postgresql+asyncpg://...` 설정 후 `uvicorn` 실행 시 자동 생성
